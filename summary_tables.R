...
for (i in 1:length(grib_names)) {
  ...
  data <- yes_data %>%
    ...
    summarise(datetime_id = min(datetime_id),
              ...
              hour = min(hour),
              count_cells = n()) %>%
    relocate(lat_id, .before = count_cells)
  latitude_summary <- bind_rows(latitude_summary, data)
  print(i)
}  
write.csv(latitude_summary,"/Volumes/Long_long/DAEN690Dragon/YES_CSV/latitude_summary.csv", row.names = FALSE)

longitude_summary <- data.frame()
for (i in 1:length(grib_names)) {
  # for (i in 4561:5016) {
  csv_name <- paste("yes_", grib_names[i], ".csv",sep="")
  yes_data <- read.csv(csv_name)
  data <- yes_data %>%
    ...
    summarise(datetime_id = min(datetime_id),
              ...
              hour = min(hour),
              count_cells = n()) %>%
    relocate(lon_id, .before = count_cells)
  longitude_summary <- bind_rows(longitude_summary, data)
  print(i)
}  
write.csv(longitude_summary,"/Volumes/Long_long/DAEN690Dragon/YES_CSV/longitude_summary.csv", row.names = FALSE)
